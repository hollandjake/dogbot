...
public class Quotes implements CommandModule, DatabaseModule {
	...
	private final String STORY_REGEX = ACTIONIFY("story ?(\\d*)");
	...
	private void quote(Message quote, String type) {
		if (quote != null) {
			String message = applyType(quote.getMessage(), type);
	...
	private void quoteCount(String query) throws NullPointerException {
		AbstractMap.SimpleEntry<String, Integer> countData = getNumberOfQuotesFromName(query);
		Integer count = Objects.requireNonNull(countData).getValue();
	...
	//region Database
	private boolean containsQuote(int threadId, Message message) {
		try {
			db.checkConnection();
	...
	private Integer getNumberOfQuotes() {
		try {
			db.checkConnection();
	...
	private AbstractMap.SimpleEntry<String, Integer> getNumberOfQuotesFromName(String name) {
		try {
			db.checkConnection();
	...
	public boolean process(Message message) throws MalformedCommandException {
		...
		if (match.equals(QUOTE_REGEX)) {
			...
			quote(quote, getType(message));
			return true;
		} else if (match.equals(STORY_REGEX)) {
			Matcher matcher = Pattern.compile(STORY_REGEX).matcher(message.getMessage());
			int n = 5;
			try {
			...
			if (n > 15) {
				...
				List<String> lateMessages = Arrays.asList(
					"Just writing the last few pages",
					"I'm still working, please don't give me a late submission",
					"I'm trying my best",
					"Please forgive me for the number of trees I have killed"
				);
				while (messages.size() < n) {
					...
					if (quote == null) {
						break;
					} else if (!quote.getMessage().isEmpty()) {
						messages.add(quote);
					}
				}
				String book = "It is time for a story" + "\n\n";
				if (messages.isEmpty()) {
					book += "It appears someones ripped the pages out or this chat has no quotes \uD83D\uDE1E" + "\n";
				} else {
					book +=
						messages.stream().map(Message::getMessage).collect(Collectors.joining("\n")) + "\n";
				}
				chatbot.sendMessage(applyType(book + "\nThe End", getType(message)));
			}
			return true;
	...
	public String getMatch(Message message) {
		...
		if (messageBody.matches(QUOTE_REGEX)) {
			return QUOTE_REGEX;
		} else if (messageBody.matches(STORY_REGEX)) {
			return STORY_REGEX;
